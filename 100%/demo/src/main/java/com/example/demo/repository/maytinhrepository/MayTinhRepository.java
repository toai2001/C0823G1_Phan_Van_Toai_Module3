...
//      + "  where mt.status = 1;";
    ...
    private final String SELECT_ID_NHAN_VIEN = "select nhan_vien.id_nhan_vien\n" +
            "from nhan_vien\n" +
            "join tai_khoan on tai_khoan.id_tai_khoan = nhan_vien.id_tai_khoan\n" +
            "where tai_khoan.tai_khoan = ?;";

    private final String CHECK_STATUS_COMPUTER = "Select may_tinh.status\n" +
            "from may_tinh\n" +
            "where may_tinh.ma_may_tinh = ?;";
...
//        mayTinhList.add(new MayTinh(idMayTinh, tenMayTinh, ramMayTinh, chip, hang, isDelete));
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                while (resultSet1.next()) {
                    tien_dich_vu = resultSet1.getDouble("total_tong_tien");
                }
                ...
                total_tien = tien_dich_vu + (10000.0 / 3600.0) * seconds;

                bangSuDung = new BangSuDung(id_bang_su_dung, thoi_gian_bat_dau, thoi_gian_ket_thuc, mayTinh, nhanVien,tien_dich_vu, total_tien, seconds, status);
    ...
    @Override
    public int getIdNhanVienByAccount(String account) {
        int idNhanVien = 0;
        Connection connection = BaseRepository.getConnection();
        try {
            PreparedStatement preparedStatement = connection.prepareStatement(SELECT_ID_NHAN_VIEN);
            preparedStatement.setString(1,account);
            ResultSet resultSet = preparedStatement.executeQuery();
            while (resultSet.next()){
                idNhanVien = resultSet.getInt("id_nhan_vien");
            }
    ...
    @Override
    public boolean checkStatusComputer(int idMay) {
        Connection connection = BaseRepository.getConnection();
        boolean check = false;
        try {
            PreparedStatement preparedStatement = connection.prepareStatement(CHECK_STATUS_COMPUTER);
            preparedStatement.setInt(1,idMay);
            ResultSet resultSet = preparedStatement.executeQuery();
            while (resultSet.next()){
                check = resultSet.getBoolean("status");
            }
