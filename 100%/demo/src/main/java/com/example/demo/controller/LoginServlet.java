...
//            TaiKhoan taiKhoan = taiKhoanService.findByUsernameAndPassWord(username, password);
            boolean checkPass = BCryptUtils.checkPassword(password,account.getMatKhau());
            if (!checkPass || account.getMatKhau() == null){
                ...
                session.setAttribute("loai_tai_khoan", account.getLoaiTaiKhoan());
