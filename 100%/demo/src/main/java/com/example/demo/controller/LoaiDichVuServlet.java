...
public class LoaiDichVuServlet extends HttpServlet {
    ...
    protected void xoaLoaiDichVu(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        int id = Integer.parseInt(request.getParameter("id"));
        List<DichVuDiKem> list1 = dichVuDiKemService.layDanhSachDichVuDiKem();
        int count = 0;
        for (DichVuDiKem dichVuDiKem : list1) {
        ...
        if (count != 0) {
            ...
            request.setAttribute("successMessage", "Xóa không thành công.");
            request.setAttribute("list", list);
            request.getRequestDispatcher("/loai_dich_vu.jsp").forward(request, response);
        } else {
